...
export default function SetupPage() {
  ...
  const fetchConfig = useCallback(async () => {
    ...
    try {
      ...
      setInitialConfigSnapshot(JSON.stringify({ entities: loadedEntities }));
      ...
      setEntities([]);
      setSelectedEntityInternalId(null);
      setInitialConfigSnapshot(JSON.stringify({ entities: [] }));
    } finally {
      setIsFetching(false);
    }
  }, [showToast]); 
  ...
  const handleSaveConfig = async () => {
    ...
    // Get current baseUrl from entityConfig or use existing baseUrl
    const currentBaseUrl = entityConfig?.baseUrl || 'https://api.axle.network';
    const configToSave: ExportConfig = { baseUrl: currentBaseUrl, entities: entitiesToSave };
    ...
    try {
      ...
      setInitialConfigSnapshot(JSON.stringify({ entities: entitiesToSave }));
    } catch (error: any) {
      ...
      <div className="flex flex-col h-full gap-2 overflow-y-auto">
        <div className="flex-shrink-0">
          <div className="flex justify-between items-start mb-6">
            ...
            <p className="text-muted-foreground">Configure your API entities and their field mappings. Use Base URLs page to manage API endpoints.</p>
          </div>
